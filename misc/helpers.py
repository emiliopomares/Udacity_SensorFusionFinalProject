...
def load_object_from_file(file_path, base_filename, object_name, frame_id=1):
    object_filename = os.path.join(file_path, os.path.splitext(base_filename)[0]
                                   + "__frame-" + str(frame_id) + "__" + object_name + ".pkl")
    # Hack to make files from Part 1 findable
    object_filename_alt = os.path.join(file_path, os.path.splitext(base_filename)[0]
                                   + "__frame-" + str(frame_id) + "__" + object_name + "_darknet_0.5.pkl")
    try:
        with open(object_filename, 'rb') as f:
            object = pickle.load(f)
            return object
    except:
        with open(object_filename_alt, 'rb') as f:
            object = pickle.load(f)
            return object
