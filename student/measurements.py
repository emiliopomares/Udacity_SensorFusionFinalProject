...
class Sensor:
    ...
    def in_fov(self, x):
        veh_x = np.zeros((4, 1))
        veh_x[0:3] = x[0:3] # was self.dim_meas instead of 3
        veh_x[3] = 1
        sensor_x = self.veh_to_sens * veh_x
        if sensor_x[0] > 0:
            angle = np.arctan(sensor_x[1]/sensor_x[0])
            res = angle >= self.fov[0] and angle <= self.fov[1]
            return res
        return False
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            if params.ESTIMATE_DIMENSIONS:
                res = np.zeros((self.dim_meas, 1))
                res[0:3] = pos_sens[0:3]
                res[3:6] = x[3:6]
                return res
            return pos_sens[0:3]
            ...
            pos_veh = np.ones((4, 1)) # homogeneous coordinates
            pos_veh[0:3] = x[0:3] 
            pos_sens = self.veh_to_sens*pos_veh # transform to camera sensor coordinates
            if pos_sens[0] == 0:
                raise ValueError("X coordinate in sensor space must be greater than zero")
            z = self.c_i - (self.f_i * pos_sens[1]/pos_sens[0])
            y = self.c_j - (self.f_j * pos_sens[2]/pos_sens[0])
            result = np.ones((self.dim_meas, 1))
            result[0] = z
            result[1] = y
            return result
    ...
    def __init__(self, num_frame, z, sensor):
        ...
        if sensor.name == 'lidar':
            ...
            self.R = np.matrix([[sigma_lidar_x**2, 0, 0], # measurement noise covariance matrix
                                ...
                                [0, 0, sigma_lidar_z**2]])
            if params.ESTIMATE_DIMENSIONS:
                self.R = np.matrix([[sigma_lidar_x**2, 0, 0, 0, 0, 0], 
                                [0, sigma_lidar_y**2, 0, 0, 0, 0], 
                                [0, 0, sigma_lidar_z**2, 0, 0, 0],
                                [0, 0, 0, params.sigma_dims**2, 0, 0],
                                [0, 0, 0, 0, params.sigma_dims**2, 0],
                                [0, 0, 0, 0, 0, params.sigma_dims**2]
                                ])
