...
def detect_objects(input_bev_maps, model, configs):
    ...
    # detections format: score, x, y, z, scalex, scaley, scalez, rotphi, rottheta, class  (10 scalars)
    # after postprocess: score, x, y, z, scalex, scaley, scalez, yaw (8 scalars)
    objects = [] 

    if(type(detections[0]) == list):
        detections = [{1:np.array([detections[0]])}]
    ...
    if len(detections[0][1]) > 0:
        ...
        for detection in detections[0][1]:
            ...
            x_min = configs.lim_x[0]
            
            y_range = configs.lim_y[1] - configs.lim_y[0]
            y_min = configs.lim_x[0]
            
            z_range = configs.lim_z[1] - configs.lim_z[0]
            z_min = configs.lim_z[0]
            
            score = detection[0]
            bev_y = detection[1] # this is y axis
            bev_x = detection[2] # this is x axis
            bev_z = detection[3] # this is height
            scale_x = detection[4]
            scale_y = detection[5]
            scale_z = detection[6]
            yaw = -detection[7]

            car_x = bev_x/configs.bev_width * x_range - x_min
            car_y = -(y_range/2 - (bev_y/configs.bev_height * y_range - y_min))
            car_z = bev_z * z_range - z_min
            car_scale_x = scale_x
            car_scale_y = scale_y/configs.bev_width * y_range
            car_scale_z = scale_z/configs.bev_height * x_range

            det_obj = [1, car_x, car_y, car_z, car_scale_x, car_scale_y, car_scale_z, yaw]
